...
**Особенности:**
- 🔐 **Ручная авторизация Steam (только 1-й раз)**: При первом запуске или когда протух токен, скрипт на 5 минут откроет браузер. Как только вы войдете через Steam, скрипт "поймает" токен, сохранит его в `skinswap_session.json` и сам закроет браузер.
